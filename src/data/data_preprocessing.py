...
def load_data(path: str) -> duckdb.DuckDBPyRelation:
    ...
    valid_rows = rel.filter("costhealthydietpppusd IS NOT NULL").aggregate("count(*)").fetchone()[0]
    if valid_rows == 0:
...
def main(input_filepath, output_filepath):
    ...
    if MY_BUCKET and os.getenv("CHEMIN_FICHIER"):
        ...
        CHEMIN_PARQUET = os.getenv("CHEMIN_PARQUET", CHEMIN_FICHIER.replace(".csv", ".parquet"))
